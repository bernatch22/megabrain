...
_VENDOR = [re.compile(p) for p in (
    ...
    r"(^|/)(vendor|third[_-]?party|thirdparty|deps|Godeps)/",
...
# `packages/` is Linguist's NuGet rule: in .NET it holds RESTORED dependencies.
# But in the JS ecosystem the very same name is the universal FIRST-PARTY
# workspace convention (nx, vite, jest, babel, typescript-eslint — nx keeps
# 4,275 source files there, and this rule proposed skipping all of them as
# "vendored"). The two worlds are distinguishable at the ROOT: a JS monorepo
# declares itself with a workspace manifest. If one exists, packages/ is code.
_PACKAGES = re.compile(r"(^|/)packages/")
_WORKSPACE_MARKERS = ("pnpm-workspace.yaml", "nx.json", "lerna.json",
                      "turbo.json", "rush.json")


@lru_cache(maxsize=64)
def _is_js_workspace(root: Path) -> bool:
    if any((root / m).exists() for m in _WORKSPACE_MARKERS):
        return True
    pj = root / "package.json"
    if pj.exists():
        try:
            import json as _json
            return "workspaces" in _json.loads(
...
def is_vendored(rel: str, root: Path | None = None) -> bool:
    """`root` enables the workspace check for `packages/`: with a JS workspace
    manifest at the root, packages/ is first-party code, not NuGet restore
    output. No root (legacy callers) keeps the conservative NuGet behavior."""
    if any(rx.search(rel) for rx in _VENDOR):
        return True
    if _PACKAGES.search(rel):
        return root is None or not _is_js_workspace(root)
    return False

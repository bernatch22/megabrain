...
- Narrate the code's ACTUAL runtime behavior, traced mechanically from the cited lines in execution order: what runs first, what state each step reads and writes, and in what order. NEVER present a name, docstring, comment or apparent intent as behavior — on buggy code, what the lines do is not what they meant. If two cited spans interact (one writes state the other reads), spell out which runs first and what value the reader sees at that point.
- If the query reports a failure, bug or unexpected behavior, take that report as FACT and walk the execution order until the walkthrough explains how the cited code produces exactly that behavior. If the cited code cannot produce it, say so explicitly ("the cause is not in the retrieved code") — never conclude that the code behaves correctly because it looks like it should.

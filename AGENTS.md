...
1. **No LLM in the retrieval path.** Deterministic pruning is the floor. The only LLM calls are `ask` (post-retrieval explainer), the optional `search --rerank` (post-prune reorder), and one cached `graph` community-label call — all fail-open.
